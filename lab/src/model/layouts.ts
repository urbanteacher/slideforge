...
import type { Anim, Box, Deck, Layer, Params, Slide } from './types';
...
function ground(st: LayoutStyle): Layer {
  return createLayer('solid', { name: 'Ground', params: { color: st.ground } });
}
...
export const LAYOUTS: LayoutDef[] = [
  ...
  },
  {
    ...
    make: (st) => slide(st, 'Statement', [
      text('Statement', 'Every chart is a choice.', cell(2, 5, 10, 6), { font: st.display, weight: '700', size: 170, color: st.ink, align: 'center', lineHeight: 1.02, tracking: -0.02 }, { type: 'words', duration: 0.7, stagger: 0.08 }),
      body(st, 'Say it, then pause.', cell(3, 12, 8, 2), 36, { align: 'center' }, 0.9),
    ]),
  },
  {
  ...
  },
  {
    ...
    make: (st) => slide(st, 'Full picture', [
      ...
      createLayer('shape', { name: 'Caption band', box: { x: 0, y: 780, w: 1920, h: 300, rot: 0 }, opacity: 0.72, params: { shape: 'rect', radius: 0, gradient: true, fill: '#000000', fill2: '#000000', angle: 90, strokeWidth: 0 } }),
      text('Caption', 'What the picture shows, in one line', cell(1, 14, 10, 2), { font: st.body, weight: '600', size: 44, color: '#ffffff' }, after(0.4)),
    ]),
  },
  {
