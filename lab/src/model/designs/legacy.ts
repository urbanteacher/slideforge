...
import { FOOT, LEFT, W, box, ground, rect, rgba, slideOf, txt, type Row } from './kit';
...
const heading = (st: LayoutStyle, title: string): Layer[] => [
  ground(st),
  txt('Heading', title, box(LEFT, 150, W - LEFT * 2, 110), { font: st.display, weight: st.displayWeight, size: 88, color: st.ink, lineHeight: 1.05, fit: 'fill' }, { type: 'rise', duration: 0.8 }),
...
export function timelineSteps(st: LayoutStyle, title: string, rows: Row[]): Slide {
  ...
  steps.forEach((s, i) => {
    ...
    layers.push(
      ...
      txt(`Step ${i + 1} — label`, s.label, box(x + 80, y + 4, W - x - 80 - LEFT, 56), { font: st.body, weight: '700', size: 48, color: st.ink }, a),
      txt(`Step ${i + 1} — text`, s.text, box(x + 80, y + 66, W - x - 80 - LEFT, rowH - 84), { font: st.body, size: 42, color: st.muted, lineHeight: 1.25 }, a),
    );
  });
...
export function panelCards(st: LayoutStyle, title: string, rows: Row[]): Slide {
  ...
  const tints = [rgba(st.accent, 0.12), rgba(st.ink, 0.07), rgba(st.ink, 0.07), rgba(st.accent, 0.12)];
  const layers = heading(st, title);
  shown.forEach((r, i) => {
    ...
    layers.push(
      ...
      txt(`${r.label} — label`, r.label, box(card.x + 40, card.y + 32, card.w - 80, 60), { font: st.body, weight: '700', size: 48, color: st.ink }, a),
      txt(`${r.label} — text`, r.text, box(card.x + 40, card.y + 106, card.w - 80, card.h - 136), { font: st.body, size: 42, color: st.muted, lineHeight: 1.25 }, a),
    );
  });
  return slideOf(title, layers, st);
}
...
export function briefCard(st: LayoutStyle, title: string, rows: Row[]): Slide {
  ...
  if (brief) layers.push(
    ...
    txt('Brief — label', brief.label, box(LEFT + 48, top + 28, W - LEFT * 2 - 96, 50), { font: st.body, weight: '700', size: 40, color: st.accent }, { type: 'fade', duration: 0.6, delay: 0.25 }),
  ...
  shown.forEach((r, i) => {
    ...
    layers.push(
      txt(`${r.label} — label`, r.label, box(b.x, b.y, b.w, 52), { font: st.body, weight: '700', size: 44, color: st.ink }, a),
      txt(`${r.label} — text`, r.text, box(b.x, b.y + 60, b.w, b.h - 60), { font: st.body, size: 42, color: st.muted, lineHeight: 1.25 }, a),
    );
  });
  return slideOf(title, layers, st);
}
