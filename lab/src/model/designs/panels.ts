...
import { FOOT, H, LEFT, W, box, eyebrow, ground, hero, part, rect, rgba, slideOf, tint, txt, type Row } from './kit';
...
export function quadrants(st: LayoutStyle, o: { title: string; rows: Row[]; notes?: string; ladder?: boolean }): Slide {
  const rows = o.rows.slice(0, 4);
  const TOP = 380;
  const layers: Layer[] = [ground(st), eyebrow(st, o.title), hero(st, 'Heading', o.title, box(LEFT, 206, W - LEFT * 2, TOP - 206 - 36), 104)];
  ...
  rows.forEach((r, i) => {
    ...
    const pad = c === 0 ? LEFT : 64;
    const bottom = Math.min(cell.y + cell.h, FOOT) - 28;
    ...
    layers.push(
      rect(`${r.label} — panel`, cell, (c + l) % 2 ? rgba(st.ink, 0.07) : st.panel, p.lead()),
      txt(`${r.label} — label`, `${o.ladder ? `${i + 1}  ` : ''}${r.label}`, box(cell.x + pad, cell.y + 40, cell.w - pad - 56, 70), { font: st.display, weight: st.displayWeight, size: 68, color: st.accent, lineHeight: 1 }, p.with(0.1)),
      txt(`${r.label} — text`, r.text, box(cell.x + pad, cell.y + 130, cell.w - pad - 56, bottom - cell.y - 130), { font: st.body, size: 50, color: st.ink, lineHeight: 1.28 }, p.with(0.2)),
    );
  });
...
export function stations(st: LayoutStyle, o: { title: string; rows: Row[]; notes?: string }): Slide {
  ...
  rows.forEach((r, i) => {
    ...
    const col = box(x0, TOP, x1 - x0, H - TOP);
    const pad = i === 0 ? LEFT : 56;
    ...
    layers.push(
      ...
      txt(`${name} — kind`, (kind ?? name).toUpperCase(), box(col.x + pad, col.y + 270, col.w - pad - 56, 56), { font: st.body, weight: '700', size: 44, color: st.ink, tracking: 0.1 }, { type: 'fade', duration: 0.6, delay: 0.25 + 0.1 * i }),
      txt(`${name} — task`, r.text, box(col.x + pad, col.y + 350, col.w - pad - 56, FOOT - col.y - 370), { font: st.body, size: 50, color: st.muted, lineHeight: 1.28 }, { type: 'fade', duration: 0.6, delay: 0.3 + 0.1 * i }),
    );
  });
  return slideOf(o.title, layers, st, o.notes);
}
...
export function connectGrid(st: LayoutStyle, o: { title: string; tiles: string[]; notes?: string }): Slide {
  ...
  tiles.forEach((t, i) => {
    ...
    layers.push(
      ...
      txt(`Tile ${i + 1} — term`, t, box(cell.x + pad, cell.y + 62, cell.w - pad - 36, Math.min(cell.y + cell.h, FOOT) - cell.y - 80), { font: st.display, weight: st.displayWeight, size: 60, color: st.ink, lineHeight: 1.05 }, a),
    );
  });
  return slideOf(o.title, layers, st, o.notes);
}
...
export function hookSplit(st: LayoutStyle, o: { title: string; prompts: string[]; notes?: string }): Slide {
  ...
  const layers: Layer[] = [
    ground(st),
    createLayer('image', { name: 'Picture — drop one here', box: box(0, 0, X, H), params: { src: '', fit: 'cover' }, anim: { type: 'fade', duration: 0.9 } }),
    txt('Eyebrow', 'LOOK CLOSELY', box(X + 80, 170, W - X - 160, 50), { font: st.body, weight: '600', size: 40, color: st.accent, tracking: 0.12 }),
    hero(st, 'Question', o.title, box(X + 80, 236, W - X - 160, 400), 112),
  ];
