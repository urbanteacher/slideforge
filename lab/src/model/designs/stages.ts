...
import { H, HEAD, LEFT, W, box, clock, eyebrow, fmtMin, ground, hero, part, rect, rgba, slideOf, tint, txt } from './kit';
...
export interface Routine { title: string; prompt: string; stages: Stage[]; notes?: string }
...
export function stageBand(st: LayoutStyle, o: Routine): Slide {
  ...
  o.stages.slice(0, n).forEach((s, i) => {
    ...
    const pad = i === 0 ? LEFT : 52;
    const inner = (dy: number, h: number, w = col.w - pad - 44) => box(col.x + pad, col.y + dy, w, h);
    ...
    layers.push(
      ...
      txt(`${s.name} — number`, String(i + 1).padStart(2, '0'), inner(52, 76, 160), { font: st.display, weight: st.displayWeight, size: 68, color: st.accent }, p.with(0.15)),
      txt(`${s.name} — name`, s.name, inner(150, 110), { font: st.display, weight: st.displayWeight, size: nameSize, color: st.ink, lineHeight: 1 }, p.with(0.2, 'rise')),
      txt(`${s.name} — task`, s.task, inner(282, col.h - 282 - 96), { font: st.body, weight: '400', size: n >= 5 ? 46 : 54, color: st.muted, lineHeight: 1.28 }, p.with(0.3)),
    );
    // Its clock: time only, flush with the number, counting from when its stage arrives.
    if (s.minutes > 0) layers.push(clock(st, `${s.name} — clock`, s.minutes, box(col.x + col.w - 44 - 190, col.y + 52, 190, 76), p.with(0.2)));
  });
...
export function stageTrack(st: LayoutStyle, o: Routine): Slide {
  ...
  stages.forEach((s, i) => {
    ...
    layers.push(
      ...
      txt(`${s.name} — task`, s.task, body(668, 300), { font: st.body, weight: '700', size: 76, color: st.ink, lineHeight: 1.15, balance: true }, p.with(0.25, 'rise')),
    );
