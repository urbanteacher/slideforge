...
import { BASE, FOOT, LEFT, PAD, TOPBAND, W, box, eyebrow, ground, hero, rect, rgba, slideOf, tint, txt } from './kit';
...
export function choiceTiles(st: LayoutStyle, g: GameDef, i: number): Slide {
  ...
  opts.forEach((o, k) => {
    ...
    if (k === q.correct) layers.push(
      ...
      txt('Answer — text', `${o}  ✓`, textBox, { font: two ? st.display : st.body, weight: two ? st.displayWeight : '700', size, color: st.ground, lineHeight: 1.15, fitGroup: 'option' }, reveal(0.05)),
    );
  });
...
export function questionBoard(st: LayoutStyle, g: GameDef): Slide {
  ...
  qs.forEach((q, i) => {
    const y = TOP + i * rowH, h = Math.min(rowH - 24, FOOT - y - 14);
    ...
    layers.push(
      ...
      txt(`Q${i + 1} — question`, q.question, box(LEFT + 130, y + 14, W - LEFT * 2 - 130, h), { font: st.body, weight: '600', size: n > 5 ? 46 : 52, color: st.ink, lineHeight: 1.2, fitGroup: 'board' }, a),
    );
  });
