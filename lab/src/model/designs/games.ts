...
import { BASE, EY, FOOT, HY, LEFT, LIFT, ON_RIGHT, PAD, RIGHT, TOPBAND, W, centred, box, eyebrow, ground, hero, rect, rgba, slideOf, tint, txt } from './kit';
...
const head = (st: LayoutStyle, g: GameDef, i: number, top: number, q: Question): Layer[] => [
  ...
  hero(st, 'Question', q.question, box(LEFT, HY, W - LEFT * 2, top - HY - (q.explanation ? 116 : 40)), 104),
];
...
export function choiceTiles(st: LayoutStyle, g: GameDef, i: number): Slide {
  ...
  // The reveal goes last: a layer after a click comes with that click, so options after the answer
  ...
  opts.forEach((o, k) => {
    ...
    const textBox = box(cell.x + pad + (two ? 0 : 130), cell.y + 32, cell.w - pad - PAD - (two ? 0 : 130), Math.min(cell.y + cell.h - 28, FOOT) - cell.y - 32);
    ...
    layers.push(
      ...
      txt(`Option ${LETTERS[k]} — text`, o, textBox, { font: two ? st.display : st.body, weight: two ? st.displayWeight : '600', size, color: st.ink, lineHeight: 1.15, fitGroup: 'option' }, a),
    );
    // The reveal: the answer's tile turns to the accent, its words reversed out of it.
    if (k === q.correct) answer.push(
      rect(`Answer — ${LETTERS[k]}`, cell, RIGHT, reveal()),
      ...(two ? [] : [txt('Answer — letter', LETTERS[k], box(cell.x + pad, cell.y + 26, 110, 110), { font: st.display, weight: st.displayWeight, size: 96, color: ON_RIGHT, lineHeight: 1 }, reveal(0.05))]),
      txt('Answer — text', `${o}  ✓`, textBox, { font: two ? st.display : st.body, weight: two ? st.displayWeight : '700', size, color: ON_RIGHT, lineHeight: 1.15, fitGroup: 'option' }, reveal(0.05)),
    );
  });
  layers.push(...answer, ...why(st, q, TOP));
...
export function choiceCards(st: LayoutStyle, g: GameDef, i: number): Slide {
  ...
  opts.forEach((o, k) => {
    ...
    layers.push(
      ...
      txt(`Option ${LETTERS[k]} — text`, o, box(card.x + 140, card.y + 20, card.w - 170, card.h - 40), { font: st.body, weight: '600', size: 54, color: st.ink, lineHeight: 1.15, fitGroup: 'option' }, a),
    );
    if (k === q.correct) answer.push(rect(`Answer — ${LETTERS[k]}`, box(card.x - 6, card.y - 6, card.w + 12, card.h + 12), rgba(RIGHT, 0.28), reveal(), 26));
  });
  layers.push(...answer, ...why(st, q, TOP));
  return slideOf(`${g.title} · ${i + 1}`, layers, st, answerNote(q));
}
...
export function typedAnswer(st: LayoutStyle, g: GameDef, i: number, cards = false): Slide {
  const q = g.questions[i];
  const TOP = 700 - LIFT;
  const layers: Layer[] = [ground(st), eyebrow(st, `${g.styleLabel} · ${i + 1} of ${g.questions.length}`), hero(st, 'Question', q.question, box(LEFT, HY, W - LEFT * 2, TOP - HY - 24), 120)];
  const band = cards ? box(LEFT, TOP, W - LEFT * 2, FOOT - TOP - 20) : box(0, TOP, W, BASE - TOP);
  layers.push(
  ...
  if (q.explanation) layers.push(txt('Answer', q.explanation, box(LEFT + (cards ? 40 : 0), TOP + 104, W - LEFT * 2 - (cards ? 80 : 0), FOOT - TOP - 130), { font: st.display, weight: st.displayWeight, size: 96, color: RIGHT, lineHeight: 1 }, reveal()));
...
export function termCard(st: LayoutStyle, g: GameDef, i: number): Slide {
  ...
  const layers: Layer[] = [
    ground(st),
    rect('Accent ground', box(0, TOPBAND, W, BASE - TOPBAND), st.accent),
    txt('Eyebrow', `EXPLAIN IT — DON’T SAY IT · ${i + 1} OF ${g.questions.length}`, box(LEFT, EY + 20, 1500, 50), { font: st.body, weight: '600', size: 40, color: st.ground, tracking: 0.14 }),
    txt('Term', q.question, box(LEFT, HY + 94, W - LEFT * 2, 420), { font: st.display, weight: st.displayWeight, size: 240, color: st.ground, lineHeight: 0.95, fit: 'fill', balance: true }, { type: 'zoomIn', duration: 0.6 }),
  ];
  if (q.explanation) layers.push(txt('Meaning', q.explanation, box(LEFT, 780 - LIFT, W - LEFT * 2, FOOT - 800 + LIFT), { font: st.body, size: 56, color: st.ground, lineHeight: 1.25 }, reveal()));
...
export function questionBoard(st: LayoutStyle, g: GameDef): Slide {
  ...
  const TOP = 360 - LIFT, n = Math.max(1, qs.length);
  const layers: Layer[] = [ground(st), eyebrow(st, `${g.styleLabel} · ${n} questions`), hero(st, 'Heading', g.title, box(LEFT, HY, W - LEFT * 2, TOP - HY - 24), 100)];
  ...
  qs.forEach((q, i) => {
    ...
    layers.push(
      ...
      centred(`Q${i + 1} — number`, String(i + 1), box(LEFT, y, 100, rowH), { font: st.display, weight: st.displayWeight, size: Math.min(88, rowH * 0.7), color: st.accent, lineHeight: 1 }, a, 0, 0),
      centred(`Q${i + 1} — question`, q.question, box(LEFT + 130, y, W - LEFT * 2 - 130, rowH), { font: st.body, weight: '600', size: n > 5 ? 46 : 52, color: st.ink, lineHeight: 1.2, fitGroup: 'board' }, a, 0, 0),
    );
  });
...
export function cubeFaces(st: LayoutStyle, g: GameDef): Slide {
  const qs = g.questions.slice(0, 6);
  const TOP = 340 - LIFT;
  const layers: Layer[] = [ground(st), eyebrow(st, 'Question cube · roll to choose'), hero(st, 'Heading', g.title, box(LEFT, HY, W - LEFT * 2, TOP - HY - 24), 96)];
  const cols = 3, cw = W / cols, ch = (BASE - TOP) / 2;
  qs.forEach((q, k) => {
    const c = k % cols, l = Math.floor(k / cols);
    const cell = box(c * cw, TOP + l * ch, cw, ch);
    const pad = c === 0 ? LEFT : PAD;
    ...
    layers.push(
      ...
      txt(`Face ${k + 1} — question`, q.question, box(cell.x + pad, cell.y + 150, cell.w - pad - PAD, Math.min(cell.y + cell.h - 24, FOOT) - cell.y - 150), { font: st.body, weight: '600', size: 50, color: st.ink, lineHeight: 1.2, fitGroup: 'face' }, w),
    );
  });
...
export function sortBoard(st: LayoutStyle, g: GameDef, i: number): Slide {
  const q = g.questions[i];
  const TOP = 500 - LIFT;
  const layers: Layer[] = [ground(st), eyebrow(st, `${g.styleLabel} · ${i + 1} of ${g.questions.length}`), hero(st, 'Prompt', q.question, box(LEFT, HY, W - LEFT * 2, TOP - HY - 24), 104)];
  ['Only the first', 'Both', 'Only the second'].forEach((name, k) => {
    const col = box(Math.round((k * W) / 3), TOP, Math.round(((k + 1) * W) / 3) - Math.round((k * W) / 3), BASE - TOP);
    const pad = k === 0 ? LEFT : PAD;
    const a = { type: 'wipeUp' as const, duration: 0.6, delay: 0.3 + 0.1 * k };
    layers.push(
      rect(`${name} — column`, col, k === 1 ? rgba(st.accent, 0.14) : tint(st, k), a),
      txt(`${name} — label`, name, box(col.x + pad, col.y + 44, col.w - pad - PAD, 90), { font: st.display, weight: st.displayWeight, size: 80, color: k === 1 ? st.accent : st.ink }, a),
    );
  });
