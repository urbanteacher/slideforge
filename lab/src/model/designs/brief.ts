...
export function briefHero(st: LayoutStyle, o: Brief): Slide {
  ...
  rows.forEach((r, i) => {
    ...
    layers.push(
      ...
      txt(`${r.label} — label`, r.label, box(col.x + pad, col.y + 52, col.w - pad - 44, 76), { font: st.display, weight: st.displayWeight, size: n >= 5 ? 60 : 76, color: st.ink, lineHeight: 1 }, p.with(0.15, 'rise')),
      txt(`${r.label} — text`, r.text, box(col.x + pad, col.y + 156, col.w - pad - 44, FOOT - col.y - 176), { font: st.body, size: n >= 5 ? 44 : 50, color: st.muted, lineHeight: 1.28 }, p.with(0.25)),
    );
  });
