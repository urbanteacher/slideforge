...
const KINDS: KindDef[] = [
  // ─── Sources ──────────────────────────────────────────────────────────────
  {
    ...
    params: [
      ...
      { key: 'wordSpacing', label: 'Word spacing', type: 'number', min: -0.2, max: 1, step: 0.01, default: 0, group: 'Spacing', decimals: 2, info: 'Space added between words, in em.' },
      FIT,
      // Effects: how the letters are painted. Every browser draws these, Safari included.
      { key: 'fillMode', label: 'Fill', type: 'select', options: [{ value: 'solid', label: 'One colour' }, { value: 'gradient', label: 'Gradient' }], default: 'solid', group: 'Effects', info: 'A gradient runs from the text colour to a second one, across the whole block of words.' },
      { key: 'color2', label: 'To colour', type: 'color', default: '#d94f2b', group: 'Effects', when: (p) => p.fillMode === 'gradient' },
      { key: 'gradientAngle', label: 'Angle', type: 'number', min: 0, max: 360, step: 1, default: 90, group: 'Effects', unit: '°', decimals: 0, info: '90° runs left to right, 180° top to bottom.', when: (p) => p.fillMode === 'gradient' },
      { key: 'outline', label: 'Outline', type: 'number', min: 0, max: 24, step: 0.5, default: 0, group: 'Effects', unit: 'px', decimals: 1, info: 'A line round every letter, outside it, so the letters keep their weight.' },
      { key: 'hollow', label: 'Outline only', type: 'bool', default: false, group: 'Effects', info: 'The letters as outlines with nothing inside. Reads best big and bold.' },
      { key: 'outlineColor', label: 'Outline colour', type: 'color', default: '#ffffff', group: 'Effects', when: (p) => Number(p.outline ?? 0) > 0 },
      { key: 'shadow', label: 'Shadow', type: 'select', default: 'none', group: 'Effects', info: 'Soft lifts the words off a picture; Hard is a solid offset copy; Long is a solid shadow falling down to the right.',
        options: [{ value: 'none', label: 'None' }, { value: 'soft', label: 'Soft' }, { value: 'hard', label: 'Hard' }, { value: 'long', label: 'Long' }] },
      { key: 'shadowColor', label: 'Shadow colour', type: 'color', default: '#000000', group: 'Effects', when: (p) => (p.shadow ?? 'none') !== 'none' },
      { key: 'glow', label: 'Glow', type: 'number', min: 0, max: 1, step: 0.01, default: 0, group: 'Effects', decimals: 2, info: 'Light round the letters, for dark slides.' },
      { key: 'glowColor', label: 'Glow colour', type: 'color', default: '#ffb347', group: 'Effects', when: (p) => Number(p.glow ?? 0) > 0 },
      { key: 'highlight', label: 'Highlighter', type: 'bool', default: false, group: 'Effects', info: 'A marker stroke behind the words, across their lower half.' },
      { key: 'highlightColor', label: 'Highlighter colour', type: 'color', default: '#ffe066', group: 'Effects', when: (p) => !!p.highlight },
      // Block: a panel the words sit in, drawn under them and moving with them.
      { key: 'block', label: 'Block', type: 'select', default: 'none', group: 'Block', info: 'Fill the box: a panel the size of the text box. Hug the words: a panel just round the words. Behind each line: a band behind every line, as a caption over a picture.',
        options: [{ value: 'none', label: 'None' }, { value: 'box', label: 'Fill the box' }, { value: 'hug', label: 'Hug the words' }, { value: 'lines', label: 'Behind each line' }] },
      { key: 'blockColor', label: 'Colour', type: 'color', default: '#f1ece4', group: 'Block', when: (p) => (p.block ?? 'none') !== 'none' },
      { key: 'blockOpacity', label: 'Opacity', type: 'number', min: 0, max: 1, step: 0.01, default: 1, group: 'Block', decimals: 2, when: (p) => (p.block ?? 'none') !== 'none' },
      { key: 'blockPad', label: 'Padding', type: 'number', min: 0, max: 160, step: 1, default: 24, group: 'Block', unit: 'px', decimals: 0, info: 'Room between the words and the edge of the block. The block grows outwards, so the words stay where they are.', when: (p) => (p.block ?? 'none') !== 'none' },
      { key: 'blockRadius', label: 'Corners', type: 'number', min: 0, max: 200, step: 1, default: 12, group: 'Block', unit: 'px', decimals: 0, when: (p) => (p.block ?? 'none') !== 'none' },
      { key: 'blockBorder', label: 'Border', type: 'number', min: 0, max: 24, step: 0.5, default: 0, group: 'Block', unit: 'px', decimals: 1, when: (p) => (p.block ?? 'none') !== 'none' },
      { key: 'blockBorderColor', label: 'Border colour', type: 'color', default: '#141414', group: 'Block', when: (p) => (p.block ?? 'none') !== 'none' && Number(p.blockBorder ?? 0) > 0 },
    ],
  },
  {
