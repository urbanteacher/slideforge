...
try {
  ...
  const lecture = await page.evaluate(() => ({
    title: SF.Editor.deck().title,
    n: SF.Editor.deck().slides.length,
    id: SF.Editor.deck().id
  }));
  ...
  /* ---- 3. Practice notes (.md) ---- */
  {
    const md = await page.evaluate(() => SF.deckToMarkdown(SF.Editor.deck()));
  ...
  const readme = [
    ...
    '',
    'Generated by `node tools/smoke-exports.mjs`.',
    ''
  ].join('\n');
