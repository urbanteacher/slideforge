...
try{
 ...
 await page.goto(url);await page.waitForFunction(()=>window.SF?.Review&&SF.Shell?.current()?.doc()?.id,null,{timeout:60000});
 ...
 // The classic canvas's Review button went with the classic studios: the dialog is opened on a stored deck directly,
 // and an import inside it must leave that stored deck as it was.
 const before=await page.evaluate(()=>{const d=SF.makeDeck('Review snapshot');d.slides=[SF.normalizeSlide({type:'title',title:'One idea'}),SF.normalizeSlide({type:'content',title:'Hidden notes',hidden:true,bullets:['For the presenter']})];SF.Store.save(d);window.__reviewId=d.id;SF.Review.open(SF.Store.get(d.id));return JSON.stringify(SF.Store.get(d.id));});
 ...
 assert.equal(await page.evaluate(()=>JSON.stringify(SF.Store.get(window.__reviewId))),before);
