...
| AC-14 | 4 | Benefits vs Limitations with team credit (games N1); Stations on a round clock; the Question Cube activity points to the game | S–M | Blocked. The cube hand-over waits on Quiz studio's AI writing the six faces (`randomchallenge` has no `question-cube` shape). Stations need a rotation-aware clock: groups are at different stations at once, and the AI guardrail wants a task per station box. The Battle needs a new spoken game style (the nine registrations): a decision for Mark |
| AC-15 | after GA-26 | Concept Card Sort and Word Splash confidence on the sort input | L | Blocked. The Card Sort compiles as a sort (tried), but Quiz studio's `compare` AI writes no statements, so "Write it" would lose its categories-and-items guardrail. Word Splash needs sort columns other than A only / Both / B only |
