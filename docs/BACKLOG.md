...
- **ENG-01, the live stage:** Present, Rehearse, Host live and Teacher Presenter run SlideForge's player, with its HUD and the room's rail, and the lab draws each lab slide live inside it (`js/lab-stage.js`, `lab-app/stage.js`): builds, word timing, transitions, on-slide controls. With the rail open, the lab draws the slide's content beside it and keeps the backdrop full size. Decided: games and activities come last, after the lesson side. [lab-engine-plan.md](lab-engine-plan.md) records both.
...
| AC-13 | with GA-15 | Tally entry for polls and scales, and a typed idea from teacher entry (A9) | M | **Deferred** 23 Sep (Mark) · two activities would use it; nothing breaks without it; a show of hands undoes the self-assessments' hold. Built for quiz questions only (GA-15). Reasoning in [the audit's section 6](activities-premium-audit.md#tally-entry-for-polls-and-scales-is-deferred) |
...
- **23 Sep 2026.** GA-19: Concept Chain is a branching map. Each Accept adds
  ...
  lost credit: the relay had no question for a chain, so an Accept's points
  never reached the scores. Two relay tests; `npm test` 527/527. Not viewed.
- **23 Sep 2026.** AC-13 deferred by Mark: tally entry stays for quiz
  questions only. Reasoning in the activities audit, section 6.
