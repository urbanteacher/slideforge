...
`ukbt-wordmark.png` is the reversed lockup (white ink) for Dark Blue and UKBT
Black; `ukbt-wordmark-dark.png` is the positive one (dark ink) for the green
and lime grounds. Both are the official 1800×425 files —
`inverse.png` and `primary.png` — not the 720×170 extract out of the brand
deck, and not the version I generated by swapping the ink myself, which was
right in structure and 2.5× short of the resolution.

`ukbt-institute-dark.svg` is the Institute equivalent, made from the official
SVG by changing the one fill that is not the chevron. Vector, so it costs
nothing and matches exactly.
