...
export function createQuizRenderer(SF, helpers) {
  ...
  /* Concept Chain as a map (GA-19). Each term the game has reached, in
     order, with the links accepted from it branching off to the right. The
     term that is up is the large one, with a dashed branch for the link
     being weighed. Earlier terms shrink, and past three only the latest
     show, so a ten-term game still fits the wall. Links carry the item they
     were accepted on (slideId), so a term that comes up twice is two rows. */
  ...
  function chainMap(slide, links, pending, open) {
    ...
    links.forEach(function (l) {
      ...
      if (!byKey[key]) { byKey[key] = { term: l.term, links: [], now: key === slide.id }; rows.push(byKey[key]); }
      byKey[key].links.push(l.link);
    });
    if (!byKey[slide.id]) rows.push({ term: slide.term || slide.question || '', links: [], now: true });
    var map = el('div', 'chain-map');
    var at = rows.findIndex(function (r) { return r.now; });
    ...
    rows.slice(hidden).forEach(function (r) {
      ...
      /* The newest links are the ones the room is talking about. */
      var from = Math.max(0, r.links.length - cap);
      if (from) branches.appendChild(el('div', 'cm-more', '+' + from + ' more'));
      r.links.slice(from).forEach(function (text, j, list) {
        branches.appendChild(el('div', 'cm-branch' + (r.now && j === list.length - 1 ? ' fresh' : ''), text));
      });
      if (r.now && open) {
    ...
    if (present === 'wordreveal') {
      ...
      if (!opts.revealed && opts.chainCommand) {
        ...
        inp.addEventListener('input', function () {
          ...
          /* Looked up each time: Accept redraws the map in place. */
