...
function hidden(s){var out=[];var points=['journey','mindmap','content','cards','split','keywords','italics','links'].includes(s.type);
 ...
 if(!['journey','quote','statement','table','introduction'].includes(s.type)&&String(s.body||'').trim())out.push({label:'extra text',field:'body',layout:'quote'});
 if(!['journey','introduction','title','section','statement','quote'].includes(s.type)&&String(s.subtitle||'').trim())out.push({label:'subtitle',field:'subtitle',layout:'title'});
