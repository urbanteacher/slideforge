...
function transition(s,a,now){
 ...
 if(a.action==='start'){
  ...
  var seconds=Number(a.seconds);if(!Number.isFinite(seconds)||seconds<0||seconds>3600)return s;
  seconds=Math.round(seconds);if(a.kind!=='task'&&!seconds)return s;
  return {kind:a.kind,title:String(a.title||({timer:'Thinking time',task:'Quick task',break:'Take a break'})[a.kind]).trim().slice(0,400),seconds:seconds,endAt:seconds?now+seconds*1000:0,timed:seconds>0,paused:false};
 }
 if(!s)return s;
 ...
 else if(a.action==='extend'&&s.timed){n.seconds=Math.min(3600,left+60);n.endAt=s.paused?0:now+n.seconds*1000;}
 return n;
}
