 ...
 *
 *   node AiAd26/contact-sheet.mjs                 # all five decks
 *   node AiAd26/contact-sheet.mjs --deck safe     # just one
 *   node AiAd26/contact-sheet.mjs --out /tmp/x    # somewhere other than AiAd26/preview
 *
...
const FILES = ['Safe', 'Smart', 'Creative', 'Responsible', 'Future']
  ...
  .map((n) => ({ name: n, file: path.join(__dirname, 'bundles', `AiAd26-${n}.sfbundle.json`) }));
...
for (const { name, file } of FILES) {
  if (!fs.existsSync(file)) {
    console.error(`missing ${path.relative(ROOT, file)} — run: node AiAd26/build.js`);
    continue;
  }
